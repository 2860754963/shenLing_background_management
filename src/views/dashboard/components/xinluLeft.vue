<template>
  <div class="">
    <el-card style="width: 732px">
      ...
      <div>
        <div id="chinaLiuxinag" style="width: 100%; height: 300px"></div>
      </div>
    </el-card>
  </div>
</template>
...
export default {
  ...
  created() {},
  mounted() {
    var myCharts = echarts.init(document.getElementById("chinaLiuxinag"));
    var option;
    const geoCoordMap = {
      ...
      浙江: [120.1, 30.16],
      getCoord(key) {
        const o = Object.keys(this).find((item) => key.indexOf(item) >= 0);
        if (o) {
          return this[o];
        }
        return [];
    ...
    const convertData = (data) => {
      ...
      for (let i = 0; i < data.length; i += 1) {
        ...
        const fromCoord = geoCoordMap.getCoord(dataItem[0].name);
        const toCoord = geoCoordMap.getCoord(dataItem[1].name);
      ...
      tooltip: {
        ...
        formatter(params) {
          if (params.data.values) {
            return `${params.marker}总数:${
              params.value[2]
            }<br/>${params.data.values
      ...
      series: [
        {
          ...
          data: labels.map((dataItem) => {
            return {
              name: dataItem.name,
              value: geoCoordMap
                .getCoord(dataItem.name)
                .concat([dataItem.value]),
              values: dataItem.values,
              itemStyle: {
                normal: {
                  color: !dataItem.value ? "#DEB887" : "#EEE8AA",
                },
              },
            };
          }),
          ...
          lineStyle: {
            normal: {
              color: (params) => {
                ...
                const b1 =
                  params.data.coords[0][0] - params.data.coords[1][0] > 0;
                const b2 =
                  params.data.coords[0][1] - params.data.coords[1][1] > 0;
                const x1 = b1 ? 1 : 0;
                const y1 = b2 ? 0 : 1;
                const x2 = b1 ? 0 : 1;
                const y2 = b2 ? 1 : 0;
